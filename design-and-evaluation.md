...
>>>>>>> 4413266924ab5a532d2ba30a50e9b10f01c64c76
---
...
optimal number of chunks to retrieve per query.

| k | Groundedness | Citation Accuracy | Latency p50 | Latency p95 |
|---|-------------|-------------------|-------------|-------------|
| 3 | *see results*| *see results*    | *see results*| *see results*|
| 5 | *see results*| *see results*    | *see results*| *see results*|
| 8 | *see results*| *see results*    | *see results*| *see results*|

*(Fill in with actual numbers after running `python src/evaluate.py`)*

**Hypothesis:** k=3 will outperform k=5 and k=8 on latency with minimal
groundedness loss, because the corpus is well-structured and the top chunk
typically contains the complete answer for single-policy questions.

**Observed during manual testing:** At k=5, 3–4 of the returned chunks for
PTO questions were from loosely related sections (approval process, holidays)
that added prompt length without contributing to the answer. k=3 returned
only the directly relevant accrual and eligibility sections.
...
*(Fill in after running evaluation — document any questions where the
answer was not grounded or citation was wrong, and what caused it)*

Example format:
- **Q__:** [question] — LLM added information not in retrieved context.
  Root cause: [explanation]. Fix applied: [what was changed].
